...
class SquaredFrobeniusLoss(nn.Module):
    def __init__(self):
        super(SquaredFrobeniusLoss, self).__init__()

    def forward(self, input, target):
        # Compute the Frobenius norm of the difference and then square it
        loss = torch.mean(torch.norm(input - target, dim=[2, 3],p='fro') ** 2,dim=1)
        return loss

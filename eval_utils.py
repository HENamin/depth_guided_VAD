...
def loss_map_re(loss_recon_map):
    return torch.mean(loss_recon_map, [1, 2, 3]).cpu().numpy()
...
def maxpatch_metric(deblur, recon, gt_static, gt_motion, patch_num=4):
    ...
    for h_idx in range(patch_num):
        for w_idx in range(patch_num):
            deblur_loss_patch = torch.mean(
                deblur_loss_map[:, (h_idx) * h_ps:(h_idx + 1) * h_ps, (w_idx) * w_ps:(w_idx + 1) * w_ps],
                [1, 2]).unsqueeze(1)
            pred_loss_patch = torch.mean(
                pred_loss_map[:, (h_idx) * h_ps:(h_idx + 1) * h_ps, (w_idx) * w_ps:(w_idx + 1) * w_ps],
                [1, 2]).unsqueeze(1)
            loss_patch = torch.mean(loss_map[:, (h_idx) * h_ps:(h_idx + 1) * h_ps, (w_idx) * w_ps:(w_idx + 1) * w_ps],
                                    [1, 2]).unsqueeze(1)
...
def min_max_np(video_loss):
    ...
    video_losses = np.ones(video_loss.shape) - (video_loss - video_losses_min * np.ones(video_loss.shape)) / (
                video_losses_max * np.ones(video_loss.shape) - video_losses_min * np.ones(video_loss.shape))
...
def calcu_result(video_results, labels_list, converse=False, norm_tag=True, smooth_tag=False):
    ...
    if norm_tag:
        for label_iter in labels_list:
            ...
            if smooth_tag:
                video_results[video_idx: video_idx + video_length] = gaussian_filter1d(
                    video_results[video_idx: video_idx + video_length], 1)
            if converse:
